...
fn check_and_divide_cells(
    ...
    for (parent_entity, cell, position, orientation, parent_timer) in divisions {
        ...
        let (child_a_color, child_a_split_interval, child_a_split_mass) = if let Some(m) = child_a_mode {
            (m.color, m.split_interval, m.split_mass)
        } else {
            (Vec3::ONE, 5.0, 1.0)
        };
        
        let (child_b_color, child_b_split_interval, child_b_split_mass) = if let Some(m) = child_b_mode {
            (m.color, m.split_interval, m.split_mass)
        } else {
            (Vec3::ONE, 5.0, 1.0)
        };
