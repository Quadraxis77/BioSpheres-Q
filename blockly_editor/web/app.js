 ...
 * 
 * Integrates all core modules for the four-mode Blockly system:
 * - ModeManager: Handles switching between Rust, WGSL, Bevy, Biospheres
...
async function initWorkspace() {
    ...
    try {
        ...
        if (typeof BiospheresGenerator !== 'undefined') {
            generators.set('biospheres', BiospheresGenerator);
        }
...
async function importCodeFiles(files) {
    ...
    try {
        ...
        if (fileContents.size === 1) {
            ...
            console.log(`[App] Imported ${allBlocks.length} total block(s) from ${fileContents.size} file(s)`);
            
            // Show cross-file references if any
            if (parser.references.size > 0) {
